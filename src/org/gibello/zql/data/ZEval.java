...
import org.faul.jql.core.JCommon;
...
public class ZEval {
	...
	/**
	 * Evaluate the contains expression
	 * @param tuple - ZTuple. The row values.
	 * @param exp - ZExpression. The where clause of the expression.
	 * @return boolean. Returns true if the evaluation of the row values is true, else returns false.
	 * @throws SQLException. If the parsing fails, an exception is thrown.
	 */
	public boolean evalContains(ZTuple tuple, ZExpression exp)
			throws SQLException {
		List<ZExpression> operands = exp.getOperands();
		List right = new Vector();
		ZConstant left = null;
		for (int i = 0; i < operands.size(); i++) {
			ZExpression stmt = operands.get(i);
			List<ZExpression> inner = stmt.getOperands();
			if (i == 0) 
				left = (ZConstant) stmt.getOperand(0);
			for (int j=1;j<inner.size();j++) {
				right.add(stmt.getOperand(j));
			}
		}
		Object value = null;
		String name = null;
		try {
			value = tuple.getAttValue(left.getValue());
		...
		for (Object x : right) {
			ZConstant cons = (ZConstant)x;
			String l = cons.getValue();
			boolean ok = false;
			...
			for (int i=0;i<valueList.size();i++) {
				...
				if (value instanceof String) {
					String r = (String)valueList.get(i);
					if (l.equalsIgnoreCase(r)==true) {
						ok = true;
						i = valueList.size();
					}
				}
				...
				if (value instanceof Long || value instanceof Double) {
					...
					if (value instanceof Long) 
						r = (double)(Long)valueList.get(i);
					else
						r = (Double)valueList.get(i);
					if (cons.getType() != ZConstant.STRING) {
						Double ll = Double.parseDouble(cons.getValue());
						if (ll.equals(r)) {
							ok = true;
							i = valueList.size();
						}
	...
	public boolean eval(ZTuple tuple, ZExp exp) throws SQLException {
		...
		if (op.equals("AND")) {
			...
			return and;
		} else if (op.equals("_CONTAINS")) {
			return evalContains(tuple, pred);
		...
		} else if (op.equals("CONTAINS") || op.equals("NOT CONTAINS")) {

			ZExpression newexp = new ZExpression("_CONTAINS");
	...
	double evalCmp(ZTuple tuple, List operands) throws SQLException {
		...
		/**
		 * Solve any embedded JSON references
		 */
		try {
			solveEmbeddedJson((ZConstant) operands.get(0), tuple);
			solveEmbeddedJson((ZConstant) operands.get(1), tuple);
		} catch (Exception error) {

		}

		o1 = evalExpValue(tuple, (ZExp) operands.get(0));
		o2 = evalExpValue(tuple, (ZExp) operands.get(1));
		
		if (o1 == null || o2 == null) {
			throw new SQLException("Operands had a null!");
		}
	...
	Object solveEmbeddedJson(ZConstant test, ZTuple tuple) throws Exception {
		if (test.getType() == 0) {
			String val = test.getValue();
			if (val.endsWith("_")) {
				Integer depth = 0;
				val = JCommon.evaluateEmbeddedJson(val, tuple);
			}
			return val;
		}
		return null;
	 ...
	 * @throws SQLException
	 */
	private boolean evalLike(ZTuple tuple, List operands) throws SQLException {
		if (operands.size() < 2) {
			throw new SQLException(
					"ZEval.evalCmp(): Trying to compare less than two values");
		}
