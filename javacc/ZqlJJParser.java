...
/*
    List v = p.SQLStatements();

    for(int i=0; i<v.size(); i++) {
      ZStatement st = (ZStatement)v.get(i);
      System.out.println(st.toString());
    }
*/
...
/* ---------------- Code Section starts here ---------------------- */
  final public List SQLStatements() throws ParseException {
  List v = new ArrayList();
  ZStatement s;
    label_1:
    while (true) {
      s = SQLStatement();
                          if(s == null) {if (true) return v;} else v.add(s);
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_COMMIT:
      case K_DELETE:
      case K_DROP:
...
// Save File Statement ::= COMMIT [WORK] [COMMENT <comment>];
  final public ZSaveFile SaveStatement() throws ParseException {
  ZSaveFile save = new ZSaveFile();
  String s;
  Token tk;
...
// LOAD File Statement ;
  final public ZLoadFile LoadStatement() throws ParseException {
  ZLoadFile load = new ZLoadFile();
  String s;
  Token tk;
...
// Use database Statement ::= USE db;
  final public ZUse UseStatement() throws ParseException {
  ZUse use = new ZUse();
  String s;
  Token tk;
...
// DropStatement ::= COMMIT [WORK] [COMMENT <comment>];
  final public ZDropTable DropStatement() throws ParseException {
  List v = new ArrayList();
  ZDropTable drop = new ZDropTable();
  String s;
...
// LockTableStatement ::= LOCK TABLE TableReference (, TableReference)*
//   IN LockMode MODE [NOWAIT];
  final public ZUnLockTable UnLockTableStatement() throws ParseException {
  ZUnLockTable ulck = new ZUnLockTable();
  List v = new ArrayList();
  String s;
...
// FromClause ::= FromItem (, FromItem)*
  final public List FromClause() throws ParseException {
  List v = new ArrayList(8);
  ZFromItem f;
    jj_consume_token(K_FROM);
    f = FromItem();
...
// Qry ::= SelectWithoutOrder | ( SelectWithoutOrder )
  ...
  final public ZExp OffsetClause() throws ParseException {
        ZExp offset;
    jj_consume_token(K_OFFSET);
    offset = SQLSimpleExpression();
          {if (true) return offset;}
    throw new Error("Missing return statement in function");
  }

  final public ZExp LimitClause() throws ParseException {
        ZExp limit;
    jj_consume_token(K_LIMIT);
    limit = SQLSimpleExpression();
          {if (true) return limit;}
    throw new Error("Missing return statement in function");
  }

  final public List OrderByClause() throws ParseException {
  List v = new ArrayList();
  ZExp e;
  ZOrderBy ob;
...
//      | SQLInClause | SQLBetweenClause | SQLLikeClause | IsNullClause | SQLContainsClause )?
...
// For prepared columns ("?")
  ...
  final public ZExpression SQLContainsClause() throws ParseException {
  ZExp eright;
  ZExpression e;
  boolean not = false;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    ...
    jj_consume_token(K_CONTAINS);
    eright = SQLSimpleExpressionOrPreparedCol();
    if(not) e = new ZExpression("NOT CONTAINS", eright);
    else e = new ZExpression("CONTAINS", eright);
    {if (true) return e;}
    throw new Error("Missing return statement in function");
  }
...
// 01/07/00 PY.Gibello - Added COUNT
  ...
  private boolean jj_3R_25() {
    ...
    if (jj_scan_token(K_CONTAINS)) return true;
    if (jj_3R_39()) return true;
    return false;
  }
